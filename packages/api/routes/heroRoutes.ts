...
// GET /characters/player/:userId
// Returns a summary list of heroes for a player
router.get('/player/:userId', requireAuth, async (req: Request, res: Response) => {
...
router.post('/:id/state', requireAuth, async (
    ...
    try {
        await pool.query(query, [JSON.stringify(newState), id]);
